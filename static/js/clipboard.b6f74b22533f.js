...
const copyContentsTextToClipboard = (text) => copyContentsToClipboard(
    { copyableText: text });

const copyContentsByIdToClipboard = (id) => copyContentsToClipboard(
    { copyableElement: document.getElementById(id) });

const copyContentsToClipboard = async ({ copyableText, copyableElement }) =>
{
...
document.getElementById("clipboard-button").addEventListener(
    "click", () => copyContentsByIdToClipboard("puzzle-contents-copyable"));
    // Example where we only want to copy a plaintext (text/plain only, no
    // text/html) string instead.
    // document.getElementById("clipboard-button").addEventListener(
    //   "click", () => copyContentsTextToClipboard("Plain text content to copy."));